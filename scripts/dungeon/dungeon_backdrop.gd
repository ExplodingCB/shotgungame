# Room border for dungeon chambers: same hull-edge look as the arena
# backdrop, but sized to whatever room the generator just built.
# dungeon_main sets `rect` each room and calls queue_redraw().
extends Node2D
...
func _draw() -> void:
	...
	_frame_band(frame, rect, Color(0.1, 0.11, 0.17))
	_frame_band(rect.grow(WALL * 0.62), rect, Color(0.18, 0.21, 0.3))
	_frame_band(rect.grow(WALL * 0.3), rect, Color(0.26, 0.3, 0.43))
	_frame_band(rect.grow(3.0), rect, Color(0.55, 0.65, 0.95))

	_frame_band(rect, rect.grow(-16.0), Color(0.45, 0.55, 1.0, 0.1))
	_frame_band(rect.grow(-16.0), rect.grow(-40.0), Color(0.45, 0.55, 1.0, 0.05))
	_frame_band(rect.grow(-40.0), rect.grow(-72.0), Color(0.45, 0.55, 1.0, 0.025))
